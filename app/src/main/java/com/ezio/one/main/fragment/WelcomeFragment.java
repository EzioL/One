...
import com.ezio.one.main.InitActivity;
import com.ezio.one.main.MainActivity;
import com.ezio.one.main.WelcomeActivity;
...
public class WelcomeFragment extends LazyLoadFragment {
    ...
    private boolean setImage() {
        if (welcome_index < welcome.length) {
            image.setBackgroundResource(welcome[welcome_index]);
            // Glide.with(getActivity()).load().into(image);
            welcome_index++;
            return true;
        } else {
            return false;
    ...
    public void onClick() {
        AppContext.getInstance().redirectToActivity(getActivity(),MainActivity.class);
